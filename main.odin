package odin_compiletime_brainfuck

PACKAGE::"odin_compiletime_brainfuck"

import "base:intrinsics"
import "core:fmt"

Integer::struct(value:int){}
Integer_PREFIX::PACKAGE+"::Integer(value:$$"
Integer_OFFSET::len(Integer_PREFIX)

RUNES::[256]string{
     0="\x00",
     1="\x01",
     2="\x02",
     3="\x03",
     4="\x04",
     5="\x05",
     6="\x06",
     7="\x07",
     8="\x08",
     9="\x09",
    10="\x0A",
    11="\x0B",
    12="\x0C",
    13="\x0D",
    14="\x0E",
    15="\x0F",
    16="\x10",
    17="\x11",
    18="\x12",
    19="\x13",
    20="\x14",
    21="\x15",
    22="\x16",
    23="\x17",
    24="\x18",
    25="\x19",
    26="\x1A",
    27="\x1B",
    28="\x1C",
    29="\x1D",
    30="\x1E",
    31="\x1F",
    32="\x20",
    33="\x21",
    34="\x22",
    35="\x23",
    36="\x24",
    37="\x25",
    38="\x26",
    39="\x27",
    40="\x28",
    41="\x29",
    42="\x2A",
    43="\x2B",
    44="\x2C",
    45="\x2D",
    46="\x2E",
    47="\x2F",
    48="\x30",
    49="\x31",
    50="\x32",
    51="\x33",
    52="\x34",
    53="\x35",
    54="\x36",
    55="\x37",
    56="\x38",
    57="\x39",
    58="\x3A",
    59="\x3B",
    60="\x3C",
    61="\x3D",
    62="\x3E",
    63="\x3F",
    64="\x40",
    65="\x41",
    66="\x42",
    67="\x43",
    68="\x44",
    69="\x45",
    70="\x46",
    71="\x47",
    72="\x48",
    73="\x49",
    74="\x4A",
    75="\x4B",
    76="\x4C",
    77="\x4D",
    78="\x4E",
    79="\x4F",
    80="\x50",
    81="\x51",
    82="\x52",
    83="\x53",
    84="\x54",
    85="\x55",
    86="\x56",
    87="\x57",
    88="\x58",
    89="\x59",
    90="\x5A",
    91="\x5B",
    92="\x5C",
    93="\x5D",
    94="\x5E",
    95="\x5F",
    96="\x60",
    97="\x61",
    98="\x62",
    99="\x63",
   100="\x64",
   101="\x65",
   102="\x66",
   103="\x67",
   104="\x68",
   105="\x69",
   106="\x6A",
   107="\x6B",
   108="\x6C",
   109="\x6D",
   110="\x6E",
   111="\x6F",
   112="\x70",
   113="\x71",
   114="\x72",
   115="\x73",
   116="\x74",
   117="\x75",
   118="\x76",
   119="\x77",
   120="\x78",
   121="\x79",
   122="\x7A",
   123="\x7B",
   124="\x7C",
   125="\x7D",
   126="\x7E",
   127="\x7F",
   128="\x80",
   129="\x81",
   130="\x82",
   131="\x83",
   132="\x84",
   133="\x85",
   134="\x86",
   135="\x87",
   136="\x88",
   137="\x89",
   138="\x8A",
   139="\x8B",
   140="\x8C",
   141="\x8D",
   142="\x8E",
   143="\x8F",
   144="\x90",
   145="\x91",
   146="\x92",
   147="\x93",
   148="\x94",
   149="\x95",
   150="\x96",
   151="\x97",
   152="\x98",
   153="\x99",
   154="\x9A",
   155="\x9B",
   156="\x9C",
   157="\x9D",
   158="\x9E",
   159="\x9F",
   160="\xA0",
   161="\xA1",
   162="\xA2",
   163="\xA3",
   164="\xA4",
   165="\xA5",
   166="\xA6",
   167="\xA7",
   168="\xA8",
   169="\xA9",
   170="\xAA",
   171="\xAB",
   172="\xAC",
   173="\xAD",
   174="\xAE",
   175="\xAF",
   176="\xB0",
   177="\xB1",
   178="\xB2",
   179="\xB3",
   180="\xB4",
   181="\xB5",
   182="\xB6",
   183="\xB7",
   184="\xB8",
   185="\xB9",
   186="\xBA",
   187="\xBB",
   188="\xBC",
   189="\xBD",
   190="\xBE",
   191="\xBF",
   192="\xC0",
   193="\xC1",
   194="\xC2",
   195="\xC3",
   196="\xC4",
   197="\xC5",
   198="\xC6",
   199="\xC7",
   200="\xC8",
   201="\xC9",
   202="\xCA",
   203="\xCB",
   204="\xCC",
   205="\xCD",
   206="\xCE",
   207="\xCF",
   208="\xD0",
   209="\xD1",
   210="\xD2",
   211="\xD3",
   212="\xD4",
   213="\xD5",
   214="\xD6",
   215="\xD7",
   216="\xD8",
   217="\xD9",
   218="\xDA",
   219="\xDB",
   220="\xDC",
   221="\xDD",
   222="\xDE",
   223="\xDF",
   224="\xE0",
   225="\xE1",
   226="\xE2",
   227="\xE3",
   228="\xE4",
   229="\xE5",
   230="\xE6",
   231="\xE7",
   232="\xE8",
   233="\xE9",
   234="\xEA",
   235="\xEB",
   236="\xEC",
   237="\xED",
   238="\xEE",
   239="\xEF",
   240="\xF0",
   241="\xF1",
   242="\xF2",
   243="\xF3",
   244="\xF4",
   245="\xF5",
   246="\xF6",
   247="\xF7",
   248="\xF8",
   249="\xF9",
   250="\xFA",
   251="\xFB",
   252="\xFC",
   253="\xFD",
   254="\xFE",
   255="\xFF"
}

String::struct(value:string){}

Empty_String::struct(length:uint){
   value:String(
      (
         ""
      )when length==0 else(
         "\x00"
      )when length==1 else(
         intrinsics.type_field_type(Empty_String(length/2),"value").value+intrinsics.type_field_type(Empty_String((length+1)/2),"value").value
      )
   )
}

Data::struct(ptr:uint,data:string){}

Unteger::struct(value:uint){}

Backtrack::struct(brainfuck:string,ip:uint,depth:uint){
   value:Unteger(
      (
         (
            ip+1
         )when depth==1 else(
            intrinsics.type_field_type(Backtrack(brainfuck,ip-1,depth-1),"value").value
         )
      )when brainfuck[ip]=='[' else(
         intrinsics.type_field_type(Backtrack(brainfuck,ip-1,depth+1),"value").value
      )when brainfuck[ip]==']' else(
         intrinsics.type_field_type(Backtrack(brainfuck,ip-1,depth),"value").value
      )
   )
}

Brainfuck_State::struct(brainfuck:string,ip:uint,ptr:uint,data:string){}

BRAINFUCK_DEBUG::false

Brainfuck::struct(brainfuck:string,ip:uint,data:string,ptr:uint,result:string){
   value:String(
      (
         result
      )when ip==len(brainfuck) else(
         intrinsics.type_field_type(Brainfuck(brainfuck,ip+1,data,ptr+1 when ptr!=len(data)-1 else 0,result when !BRAINFUCK_DEBUG else (result+(intrinsics.type_canonical_name(Brainfuck_State(brainfuck,ip,ptr,data))+"\n"))),"value").value
      )when brainfuck[ip]=='>' else(
         intrinsics.type_field_type(Brainfuck(brainfuck,ip+1,data,ptr-1 when ptr!=0 else len(data)-1,result when !BRAINFUCK_DEBUG else (result+(intrinsics.type_canonical_name(Brainfuck_State(brainfuck,ip,ptr,data))+"\n"))),"value").value
      )when brainfuck[ip]=='<' else(
         intrinsics.type_field_type(Brainfuck(brainfuck,ip+1,data[:ptr]+RUNES[(i16(data[ptr])+1)%%256]+data[ptr+1:],ptr,result when !BRAINFUCK_DEBUG else (result+(intrinsics.type_canonical_name(Brainfuck_State(brainfuck,ip,ptr,data))+"\n"))),"value").value
      )when brainfuck[ip]=='+' else(
         intrinsics.type_field_type(Brainfuck(brainfuck,ip+1,data[:ptr]+RUNES[(i16(data[ptr])-1)%%256]+data[ptr+1:],ptr,result when !BRAINFUCK_DEBUG else (result+(intrinsics.type_canonical_name(Brainfuck_State(brainfuck,ip,ptr,data))+"\n"))),"value").value
      )when brainfuck[ip]=='-' else(
         /*TODO , idc*/intrinsics.type_field_type(Brainfuck(brainfuck,ip+1,data,ptr,result when !BRAINFUCK_DEBUG else (result+(intrinsics.type_canonical_name(Brainfuck_State(brainfuck,ip,ptr,data))+"\n"))),"value").value
      )when brainfuck[ip]==',' else(
         intrinsics.type_field_type(Brainfuck(brainfuck,ip+1,data,ptr,(result+RUNES[data[ptr]]) when !BRAINFUCK_DEBUG else (result+(intrinsics.type_canonical_name(Brainfuck_State(brainfuck,ip,ptr,data))+"\n"))),"value").value
      )when brainfuck[ip]=='.' else(
         /*ignore [*/intrinsics.type_field_type(Brainfuck(brainfuck,ip+1,data,ptr,result when !BRAINFUCK_DEBUG else (result+(intrinsics.type_canonical_name(Brainfuck_State(brainfuck,ip,ptr,data))+"\n"))),"value").value
      )when brainfuck[ip]=='[' else(
         intrinsics.type_field_type(Brainfuck(brainfuck,ip+1 when data[ptr]==0 else intrinsics.type_field_type(Backtrack(brainfuck,ip-1,1),"value").value,data,ptr,result when !BRAINFUCK_DEBUG else (result+(intrinsics.type_canonical_name(Brainfuck_State(brainfuck,ip,ptr,data))+"\n"))),"value").value
      )when brainfuck[ip]==']' else(
         /*ignore anything else*/intrinsics.type_field_type(Brainfuck(brainfuck,ip+1,data,ptr,result when !BRAINFUCK_DEBUG else (result+(intrinsics.type_canonical_name(Brainfuck_State(brainfuck,ip,ptr,data))+"\n"))),"value").value
      )
   )
}

brainfuck::#force_inline proc($brainfuck:string,$size:uint)->string{
   init_string::intrinsics.type_field_type(Empty_String(size),"value").value
   return intrinsics.type_field_type(Brainfuck(brainfuck,0,init_string,0,""),"value").value
}

Escaper::struct(result:string,first:bool){
   value:String(
      (
         (
            "[]"
         )when len(result)==0 else(
            "["+intrinsics.type_canonical_name(Integer(int(result[0])))[Integer_OFFSET:len(intrinsics.type_canonical_name(Integer(int(result[0]))))-1]+intrinsics.type_field_type(Escaper(result[1:],false),"value").value
         )
      )when first else(
         (
            "]"
         )when len(result)==0 else(
            ","+intrinsics.type_canonical_name(Integer(int(result[0])))[Integer_OFFSET:len(intrinsics.type_canonical_name(Integer(int(result[0]))))-1]+intrinsics.type_field_type(Escaper(result[1:],false),"value").value
         )
      )
   )
}

brainfuck_as_escaped::#force_inline proc($brainfuck:string,$size:uint)->string{
   when BRAINFUCK_DEBUG{
      return "Sorry escaping debug will recurse to hell"
   }else{
      init_string::intrinsics.type_field_type(Empty_String(size),"value").value
      result::intrinsics.type_field_type(Brainfuck(brainfuck,0,init_string,0,""),"value").value
      return intrinsics.type_field_type(Escaper(result,true),"value").value
   }
}

main::proc(){
   fmt.printfln("%q",brainfuck("+++++[.-].",1))
   fmt.println(brainfuck_as_escaped("+++++[.-].",1))
   fmt.println(intrinsics.type_field_type(Brainfuck(".>.>..>.>.>.>.>.>.>.>.",0,"Helo World!",0,""),"value").value)
   fmt.println(intrinsics.type_field_type(Escaper("ABCDEFGHIJKLMNOPQRSTUVWXYZ",true),"value").value)
}

/*
>++++++++[<+++++++++>-]<.>++++[<+++++++>-]<+.+++++++..+++.>>++++++[<+++++++>-]<++.------------.>++++++[<+++++++++>-]<+.<.+++.------.--------.>>>++++[<++++++++>-]<+.
*/
